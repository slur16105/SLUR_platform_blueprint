...
class OrderCompleteScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final grandTotal = result['grand_total'] as int;
    final depositAccount = result['deposit_account'] as String;
    final depositDueAt = result['deposit_due_at'] as String;
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('주문 완료'), automaticallyImplyLeading: false),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const Center(
              child: Text('주문이 접수되었습니다',
                  style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
            ),
            const SizedBox(height: 8),
            Center(
              child: Text('입금이 확인되면 주문이 확정됩니다.',
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  children: [
                    infoRow('총 결제 금액', '${formatWon(grandTotal)}원'),
                    infoRow('입금 계좌', depositAccount),
                    infoRow('입금 기한', _formatDueAt(depositDueAt)),
                  ],
                ),
              ),
            ),
