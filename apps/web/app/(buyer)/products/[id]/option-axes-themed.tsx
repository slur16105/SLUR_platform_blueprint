...
/* 옵션 축 칩 + 선택 결과 줄 (FR-9·10, UX-DR8) — **새 테마 판**.

   🚨 이 파일은 option-axes.tsx의 **로직을 그대로 복사**하고 클래스(표현)만 바꾼 것이다.
      축 판정·기준축(pivot)·선택 유지 규칙은 검증된 동작이므로 손대지 않는다.
      옛 화면(아직 이전 전)은 계속 option-axes.tsx를 쓴다 — 둘은 서로 독립이다.
      상세 화면 이전이 끝나 옛 파일을 쓰는 곳이 없어지면 그때 옛 파일을 지운다.
...
export default function OptionAxes({
  ...
  const renderAxis = (axis: AxisNo, label: string, values: string[], sel: string | null) => (
    <div className="border-t border-border py-6">
      <span className="mb-3 block text-[13px] font-semibold uppercase tracking-wide text-muted-foreground">
        {label}
      </span>
        ...
        {values.map((value) => {
          ...
          return (
            <button
              key={value}
              type="button"
              /* 🚨 disabled 속성을 쓰지 않는다(포커스·낭독에서 사라진다) — aria-disabled + 클릭 무시 */
              className={`flex items-center gap-2 border px-4 py-2.5 text-[14px] transition-colors ${
                isSelected
                  ? "border-foreground bg-foreground text-background"
                  : "border-border hover:border-foreground"
              } ${state.disabled ? "cursor-default text-muted-foreground opacity-50" : ""}`}
              aria-pressed={isSelected}
              aria-disabled={state.disabled ? "true" : undefined}
              onClick={() => pick(axis, value, state)}
            >
              <span className={state.disabled ? "line-through" : undefined}>{value}</span>
              {state.sub ? <span className="text-[11px] opacity-70">{state.sub}</span> : null}
            </button>
          );
        })}
      </div>
  ...
  if (!has1 && !has2) {
    return allSoldOut ? (
      <div className="border-t border-border py-6">
        <span className="text-[14px] text-muted-foreground">{MSG_ALL_SOLD_OUT}</span>
      </div>
    ) : null;
  }
  ...
  return (
    <section>
      ...
      {allSoldOut ? (
        ...
        <div className="flex flex-wrap items-center gap-3 border-t border-border py-5">
          <span className="text-[13px] uppercase tracking-wide text-muted-foreground">선택</span>
          <span className="text-[15px] font-medium">{comboText}</span>
          {/* 🚨 주문 상태 modifier(m_waiting·m_finished)를 빌려 쓰지 않는다 — 액센트가 붙는 상태는
              주문의 `입금대기` 하나뿐이며(DESIGN 상태 라벨), 옵션의 `구매 가능`은 그 상태가 아니다. */}
          <span
            className={`text-[13px] font-medium ${selected.purchasable ? "text-muted-foreground" : "text-accent"}`}
          >
        ...
        <div className="border-t border-border py-5">
          <span className="text-[14px] text-muted-foreground">{MSG_CHOOSE}</span>
        </div>
      )}
    </section>
  );
