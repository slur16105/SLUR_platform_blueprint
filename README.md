...
| 영역 | 화면 |
| --- | --- |
| 구매자 | 홈 · 상품 상세 · 장바구니 · 주문서 · 주문 내역/상세 · 반품·교환 · 안내 도우미 · 1:1 문의 · 공지사항 · 내 정보(배송지 주소록) · 로그인 |
...
#### 안내 도우미 (로컬 검증 단계)
FAQ에서 답을 찾지 못했을 때 그 자리에서 물어봅니다. **FAQ·이용약관 문서에 있는 내용으로만 답하고, 근거를 함께 보여줍니다.** 문서에 없는 것(적립금·쿠폰처럼 없는 기능), 개별 처리가 필요한 것(환불·결제·배송 조회)은 **지어내지 않고 1:1 문의로 연결합니다.**

평가 문항 32개 기준 **정답률 100%, 근거 없는 답변 0건**입니다. 검색(bge-m3)과 생성(로컬 LLM)을 나눠 측정하며, 근거 점수가 낮거나 개별 처리 요구로 판정되면 모델을 부르기 전에 문의로 넘깁니다.

> ⚠️ 로컬 LLM(Ollama)을 쓰므로 **배포에는 포함되지 않습니다.** 기본값이 꺼짐이라 배포 환경에서는 이 영역이 화면에 그려지지 않습니다.

![안내 도우미](docs/screenshots/buyer-faq-chat.png)

#### 1:1 문의
통신판매중개자의 소비자 불만·분쟁 처리 창구입니다. 운영자 답변이 같은 화면에 이어 붙습니다. **안내 도우미가 답하지 못한 질문이 이곳으로 이어집니다.**
