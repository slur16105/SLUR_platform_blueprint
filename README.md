...
> **스크린샷** — 각 화면 캡처는 아래 [촬영 가이드](#스크린샷-촬영-가이드)에 파일명·경로·계정으로 정리돼 있습니다. 로컬 스택(`localhost:3000`)을 띄우고 촬영해 `docs/screenshots/`에 저장한 뒤 각 항목에 이미지를 넣으면 됩니다. (데모 계정·시드는 [빠른 시작](#빠른-시작--자체-서버-로컬-검증) 참고)
