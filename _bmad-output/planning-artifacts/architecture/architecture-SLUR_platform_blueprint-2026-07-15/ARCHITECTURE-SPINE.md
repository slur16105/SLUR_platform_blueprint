...
| 항목 | 미루는 이유 · 재개 조건 |
| --- | --- |
| PG 연동 구조 (결제 승인 웹훅, 가상계좌) | 오픈 게이트 시점. AD-3의 전이표에 `pending_payment→paid` 전이 주체만 교체되도록 설계돼 있음. **주의: 전이는 대비됐으나 거래 레코드(`payments`/`refunds`·환불 금액)는 스키마에 자리가 없다** — 2026-07-30 표준 대조 감사 참조 |
| 정산·수수료 데이터 모델 | PG와 한 묶음 (PRD §8). `sellers`에 지급계좌 컬럼도 없음 |
| ERD 업계 표준 공백 (P0: 약관 동의 이력·주문번호 UNIQUE·`payments`/`refunds`·반품(청약철회) / P1: `addresses`·무료배송 임계·`shipments`·SKU·재고 원장) | 2026-07-30 표준 대조 감사 결과. v1 완주에는 무영향이나 실구매자 오픈 시 한 번에 청구된다. 전량 AD-9 승인 게이트 대상 — 착수 전 `implementation-artifacts/deferred-work.md` "ERD 업계 표준 대조 감사" 절을 먼저 읽을 것 |
| soft reservation(재고 예약), 다중 창고 | 대형 트래픽 문제 — v1 규모에서 불필요 (AD-4가 자리 지킴) |
| 검색·알림·리뷰의 아키텍처 | PRD가 v1 제외 — 필요 확인 시 도메인 패키지 추가로 흡수 |
| 콘텐츠·CS 지면 (공지사항·1:1 문의·FAQ·상품 Q&A) | 테이블·라우트 0건. 새 테이블 추가만으로 흡수되므로 구조 위험은 낮으나, **1:1 문의(전자상거래법 §20-2 소비자 불만 처리 — 중개자 면책의 전제)와 공지(약관 변경 고지 의무)는 법정 의무**다. 약관 버전 + 동의 이력과 한 스토리로 묶을 것 — 상세는 `deferred-work.md` "콘텐츠·CS 지면 공백" |
