...
- **상품 수정 화면** — 백엔드는 `PATCH /sellers/products/{id}`로 이름·가격·설명·카테고리·상태를 받지만, 콘솔에는 목록의 상태 변경만 있다. 등록 후 가격·설명을 고치려면 화면이 필요하다. 이번 범위는 "다듬기"라 신규 화면은 만들지 않았다 — **다음 판매자 작업의 1순위 후보**.
