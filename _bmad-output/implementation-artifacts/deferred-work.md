...
giftpop 관리자 시안(`apps/web/public/giftpop-admin/page/admin-orders.html`)에는 있으나 이번 이식에서 넣지 않은 항목. 판정 기준은 두 가지 — **① SLUR의 중개 모델과 맞는가 ② 딛고 설 데이터·기능이 이미 있는가** (Slur 승인, 2026-07-30). 이번에 넣은 것은 상태 탭·기간 필터(`period`=today/7d/30d, KST 자정 경계)·필터 초기화·페이지 번호다.
