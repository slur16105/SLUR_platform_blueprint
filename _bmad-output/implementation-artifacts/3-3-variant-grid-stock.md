...
- [x] Task 2: API — PUT /sellers/products/{id}/variants (전체 교체 방식 — 그리드 저장과 1:1), product create에 stock 필드(기본 조합 1개 자동 생성, AC 2). 본인 상품만 — 404로 응답 (존재 노출 방지, 스펙 갱신)
...
- [x] Task 6: 배포 + 프로덕션 E2E (조합 상품 등록→조합 확인→정리)
...
- R6 code 시드: `duplicate_variant`(422). 상한 초과는 validation_error 422로 수용 (이미지와 동일 패턴)
...
- 프로덕션 E2E: 옵션 상품 등록→2조합(블랙 M/L, L +1000원) 저장·조회, 정리 완료. 배포 1회 일시적 DB 타임아웃 → 재배포로 해결
- 리뷰 반영: 재시도 중복 등록 방지(생성 id 보관), 그리드 정합성 검증(축 이름 통일·이름-값 짝·옵션없음 1행), 음수 실판매가 차단, IntegrityError 원인 구분, buildRows dedupe, NaN 방어, 술어 docstring에 조건부 UPDATE 계약 명시
- **보류(중요)**: PUT 전체 교체는 variant id를 전멸시킴 — 장바구니가 variant를 참조하는 4.1 착수 전에 교체 전략(upsert vs FK 정책) 재검토 필수 (R5)

### File List

- apps/api/app/products/{models,schemas,service}.py, app/sellers/router.py, app/core/config.py, alembic/versions/755424d010a0, tests/test_variants.py
- apps/web/app/seller/products/new/{page.tsx,new.css}, app/api/sellers/products/route.ts
